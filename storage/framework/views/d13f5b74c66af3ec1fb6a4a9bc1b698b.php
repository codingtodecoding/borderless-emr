...
<script>
    // Update template download link and sample file links based on selected campaign type
    document.getElementById('campaign_type_id').addEventListener('change', function() {
        ...
        if (campaignTypeId) {
            ...
            // Hide all sample links
            for (let i = 7; i <= 10; i++) {
                const link = document.getElementById('sampleLink' + i);
                if (link) link.style.display = 'none';
            }

            // Show only the selected campaign type's sample link
            const selectedLink = document.getElementById('sampleLink' + campaignTypeId);
            if (selectedLink) {
                selectedLink.style.display = 'inline-block';
            }
        } else {
            // Hide sample files container if no campaign type is selected
            document.getElementById('sampleFilesContainer').style.display = 'none';
        }
    });

    // Initialize on page load with selected value
