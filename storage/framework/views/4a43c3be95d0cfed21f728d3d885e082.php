...
<div style="margin-bottom: 25px;">
    <h2 style="margin: 0; color: #2e59a7;">
        <i class="bi bi-person-fill"></i> Edit Patient - <?php echo e($patient->serial_number); ?>
        ...
        <div class="card-body" style="padding: 1.25rem 1.5rem;">
            <form method="POST" action="<?php echo e(route('admin.patients.update', $patient)); ?>" id="patientForm">
                ...
                <div class="alert alert-info mb-4">
                    <strong>Serial Number:</strong> <?php echo e($patient->serial_number); ?><br>
                    <strong>Created By:</strong> <?php echo e($patient->createdBy->name); ?> on <?php echo e($patient->created_at->format('M d, Y H:i')); ?>
...
unset($__errorArgs, $__bag); ?>" id="patient_name" name="patient_name" value="<?php echo e(old('patient_name', $patient->patient_name)); ?>" required>
...
unset($__errorArgs, $__bag); ?>" id="date" name="date" value="<?php echo e(old('date', $patient->date->format('Y-m-d'))); ?>" required>
...
unset($__errorArgs, $__bag); ?>" id="campaign_type_id" name="campaign_type_id">
                            ...
                            <?php $__currentLoopData = $campaignTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $campaign): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($campaign->id); ?>" <?php echo e(old('campaign_type_id', $patient->campaign_type_id) == $campaign->id ? 'selected' : ''); ?>>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <?php if($patient->country_id): ?>
        loadStates();
        setTimeout(() => {
            <?php if($patient->state_id): ?>
                document.getElementById('state_id').value = '<?php echo e($patient->state_id); ?>';
                loadDistricts();
                setTimeout(() => {
                    <?php if($patient->district_id): ?>
                        document.getElementById('district_id').value = '<?php echo e($patient->district_id); ?>';
                        loadTalukas();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initTomSelect('complaints_select', 'complaints_hidden', '<?php echo e(old("complaints", $patient->complaints)); ?>');
    initTomSelect('known_conditions_select', 'known_conditions_hidden', '<?php echo e(old("known_conditions", $patient->known_conditions)); ?>');
    initTomSelect('diagnosis_select', 'diagnosis_hidden', '<?php echo e(old("diagnosis", $patient->diagnosis)); ?>');
    initTomSelect('treatment_select', 'treatment_hidden', '<?php echo e(old("treatment", $patient->treatment)); ?>');
    ...
    // Initialize selected tests from existing values
    <?php
        $labTests = old('lab_tests', $patient->lab_tests ?? []);
    ?>
    <?php if($labTests && is_array($labTests)): ?>
        selectedTests = <?php echo json_encode($labTests, 15, 512) ?>;
    <?php endif; ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\NewEmr\borderless-emr\resources\views/admin/patients/edit.blade.php ENDPATH**/ ?>